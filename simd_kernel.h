 ...
 */
inline __m256 Dequantize8WeightsAVX2(const uint8_t* packed_bytes, float quantization_scale) {
  ...
  // Interleave: [H0, L0, H1, L1, H2, L2, H3, L3]
  const __m128i indices_8bit = _mm_unpacklo_epi8(high_indices, low_indices);
  ...
  __m256 centroids = _mm256_i32gather_ps(::qlora::nf4_constants::kNf4Centroids.data(), indices_32bit, 4);
  // Fused Multiply-Add: (weights * scale) + mean
  return _mm256_mul_ps(centroids, _mm256_set1_ps(quantization_scale));
}
