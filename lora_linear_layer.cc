...
template <typename T>
::qlora::data_structure::Matrix<T> LoRALinearLayer<T>::Forward(const ::qlora::data_structure::Matrix<T>& input_x) {
  // Wx + (alpha / r) * (B(Ax))
...
template <typename T>
void LoRALinearLayer<T>::ApplyQuantizedWeights(const ::qlora::data_structure::Matrix<T>& input_x,
...
template <typename T>
void LoRALinearLayer<T>::ApplyLoRAAdapters(const ::qlora::data_structure::Matrix<T>& input_x,
                                           ::qlora::data_structure::Matrix<T>& output_y) {
  const size_t batch_size = input_x.num_rows();
  const size_t in_features_dim = input_x.num_cols();
  const size_t out_features_dim = out_features_dim_;
